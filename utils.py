...
def rgb_ssim(img1, img2):
    ...
    for i in range(3):
        ...
        ssim += (2 * avg_x * avg_y + 6.5025) * (2 * cov_xy + 58.5225) / (avg_x ** 2 + avg_y ** 2 + 6.5025) / (std_x ** 2 + std_y ** 2 + 58.5225)
...
def grayscale_ssim(img1, img2):
    ...
    ssim = (2 * avg_x * avg_y + 0.0255) * (2 * cov_xy + 0.2295) / (avg_x ** 2 + avg_y ** 2 + 0.0255) / (std_x ** 2 + std_y ** 2 + 0.2295)

    return ssim


def SSIMLoss(Y, Y_pred, device):
    loss = 1 - grayscale_ssim(Y_pred, Y)
